...
def main():
  ...
  # JIT-compiled forward step: same shapes every tick -> fused kernels after warmup
  @TinyJit
  def step_forward(frame_oh, activations):
    action, attn = player(activations)
    logits, acts_raw = world(frame_oh, action)
    return logits, acts_raw, action
  ...
  try:
    while True:
      ...
      # detach between ticks — O(1) backward, not O(tick)
      activations = activations.detach()

      # one-hot encode frame (outside JIT since randint shape varies on first call)
      frame_oh = Tensor.one_hot(frame, N_TILES).float()

      # JIT-compiled forward: player + world in one fused kernel graph
      logits, activations_raw, action = step_forward(frame_oh, activations)
      activations = activations_raw.unsqueeze(0)
      ...
      next_frame_soft = (soft * tile_vals.reshape(1, 1, N_TILES)).sum(axis=-1)

      frame_buf.append(next_frame_soft)
      if len(frame_buf) > FRAME_BUF: frame_buf = frame_buf[-FRAME_BUF:]

      # hard frame for next tick (no grad)
      ...
      if len(frame_buf) >= 4:
        ...
        opt.step()
        # realize all params to force GPU execution before next tick
        for p in all_params: p.realize()
        metrics.update({'surprise': float(s.numpy()), 'coherence': float(c.numpy()), 'persistence': float(pers.numpy()), 'loss': float(loss_val.numpy())})
        # detach old frames, keep last 2 live for gradient flow
