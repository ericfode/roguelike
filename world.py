...
class WorldNet:
  ...
  def forward(self, frame_onehot, action):
    """frame_onehot: [H, W, N_TILES] float, action: [ACTION_DIM] float"""
    x = (frame_onehot @ self.embed).permute(2, 0, 1).unsqueeze(0)  # [1, HIDDEN, H, W]
    r = x
  ...
  def __call__(self, frame_onehot, action): return self.forward(frame_onehot, action)
